...
class Recommendation(BaseModel):
    ...
    Extended with optional appointment operation fields to support the
    appointment agentic tools integration. These fields are populated when
    appointment tools are invoked during the navigation agent execution.
    """
    recommendation_id: str
    mrn: Optional[str] = None
    """Medical Record Number from the upstream Patient/Hospital System.
    Passed through from the navigation request; never generated or modified."""
    
    decision: CareDecision
    top_providers: List[ProviderCandidate]
    
    # Optional appointment operation fields (populated from agent_result)
    appointment_operations_performed: Optional[bool] = None
    """True if any appointment tools were invoked during execution."""
    
    appointment_results: Optional[List[dict]] = None
    """Append-only log of appointment operations if any were performed."""
    
    # Appointment Agent response (auto-triggered after navigation)
    appointment_agent_response: Optional[str] = None
    """Conversational response from the Appointment Agent presenting nearby
    providers to the patient. Populated automatically after navigation."""
    
    nearby_providers: Optional[List[dict]] = None
    """Provider list returned by the Appointment Agent's provider-search tool.
    Each entry: {provider_id, provider_name, facility_name, type, address,
