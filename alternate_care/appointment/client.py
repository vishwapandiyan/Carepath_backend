...
from app.services.alternate_care.models.schemas import AppointmentSlot, BookingRequest, BookingConfirmation
...
from app.services.alternate_care.appointment.adapter import SharedAppointmentAdapter
from app.services.alternate_care.config.settings import APPOINTMENT_AGENT_BASE_URL
    ...
    def book(
        ...
        The external payload is built by SharedAppointmentAdapter:
            {actor, patient_id, request:{intent, specialty?, preferred_date?,
             preferred_time?, provider_id?, slot_id?}, patient_context?}

        Critically:
          - recommendation_id is NEVER forwarded (INTERNAL field).
          - care_type/destination is NOT in the external request (CONTRACT GAP).

        STEP 5 FIX: provider_id and slot_id were previously dropped here
        (CONTRACT GAP) even though BookingRequest carries both. They are
        now forwarded to the adapter so the booking target is unambiguous.
