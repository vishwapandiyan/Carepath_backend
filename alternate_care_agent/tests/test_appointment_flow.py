...
External boundaries mocked:
    ...
    - engine.explainer.explain_decision
        (avoids live Anthropic LLM call)
...
NOT mocked:
    ...
    - CareClassifier       (exercises the real rule engine)
    - LangGraph graph      (exercises the real pipeline; LLM node is mocked)
...
def client():
    """
    FastAPI TestClient with both external boundaries patched:
      - provider discovery → deterministic list
      - LLM explainer     → deterministic string
    """
    with (
        patch(
            "location.provider_discovery.find_nearby_providers",
...
def dental_client():
    ...
    Patches location.provider_discovery.find_nearby_providers — the single
    canonical patch target now that ranking_agent.py accesses the function
    through the module object rather than a copied direct binding.
    """
    with (
        patch(
            "location.provider_discovery.find_nearby_providers",
...
def pcp_client():
    ...
    Verifies the full flow: patient input → PCP routing → location/discovery
    → ranking → Gemini explanation → structured response.
    """
    with (
        patch(
            "location.provider_discovery.find_nearby_providers",
...
def test_navigate_telehealth_destination_no_provider_lookup(monkeypatch):
    ...
    - provider discovery is NOT called (TELEHEALTH has no physical location)
    - top_providers is empty []
    - Gemini explanation is present (mocked)
    - response is a valid Recommendation with recommendation_id
    - Gemini is NOT inventing provider locations (none are returned)
    """
    ...
    # TELEHEALTH must NOT trigger a physical provider search
    assert discovery_call_count["n"] == 0, (
        f"find_nearby_providers must NOT be called for TELEHEALTH; "
