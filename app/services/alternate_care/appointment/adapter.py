...
from app.services.alternate_care.models.schemas import AppointmentSlot
...
class SharedAppointmentAdapter:
    ...
    def build_availability_request(
        ...
        date_range:
        provider_id:
            The provider's OSM ID (e.g. "osm:way:594121613").
            Included in the request sub-object when provided.
            STEP 5 FIX: This parameter was previously omitted (CONTRACT GAP).
        ...
        # STEP 5 FIX: Include provider_id in the request when supplied
        if provider_id:
    ...
    def build_book_request(
        ...
        STEP 5 FIX: provider_id and slot_id were previously omitted
        entirely (CONTRACT GAP), even though BookingRequest carries both.
        They are now included in the request sub-object when supplied, so
        the external/internal Appointment Service can identify exactly
        which provider and slot to book.
        """
        ...
        if preferred_time:
            inner["preferred_time"] = preferred_time
        # STEP 5 FIX: include provider_id / slot_id when supplied
        if provider_id:
            inner["provider_id"] = provider_id
        if slot_id:
