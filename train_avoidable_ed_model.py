"""
Train Avoidable ED Prediction Model (scikit-learn 1.9.0 Compatible)
...
Trains Random Forest Classifier and saves compatible pickle bundle to:
...
print("Generating clinical dataset with 2,500 patient records...")
...
data = {
    ...
    'primary_symptom_category': np.random.choice(symptom_categories, N_SAMPLES),
    'pain_level_self_reported': np.random.randint(0, 11, N_SAMPLES),
    'pain_onset': np.random.choice(onsets, N_SAMPLES),
    ...
    'pain_radiating': np.random.choice([0, 1], N_SAMPLES, p=[0.75, 0.25]),
    'symptom_trend': np.random.choice(trends, N_SAMPLES),
    
    # Red flags
    'flag_shortness_of_breath': np.random.choice([0, 1], N_SAMPLES, p=[0.85, 0.15]),
    'flag_chest_pain_sweating_nausea': np.random.choice([0, 1], N_SAMPLES, p=[0.88, 0.12]),
    'flag_loss_of_consciousness': np.random.choice([0, 1], N_SAMPLES, p=[0.95, 0.05]),
    'flag_confusion_altered_mental_state': np.random.choice([0, 1], N_SAMPLES, p=[0.93, 0.07]),
    'flag_uncontrolled_bleeding': np.random.choice([0, 1], N_SAMPLES, p=[0.96, 0.04]),
    'flag_severe_allergic_reaction': np.random.choice([0, 1], N_SAMPLES, p=[0.94, 0.06]),
    'flag_stroke_signs': np.random.choice([0, 1], N_SAMPLES, p=[0.97, 0.03]),
    'flag_vomiting_blood_or_blood_in_stool': np.random.choice([0, 1], N_SAMPLES, p=[0.96, 0.04]),
    'flag_high_fever_stiff_neck_rash': np.random.choice([0, 1], N_SAMPLES, p=[0.92, 0.08]),
    'flag_severe_dehydration': np.random.choice([0, 1], N_SAMPLES, p=[0.90, 0.10]),
    
    # Vitals
    'systolic_bp': np.random.randint(85, 175, N_SAMPLES),
    'diastolic_bp': np.random.randint(55, 105, N_SAMPLES),
    'heart_rate': np.random.randint(55, 125, N_SAMPLES),
    'respiratory_rate': np.random.randint(12, 28, N_SAMPLES),
    'temperature': np.round(np.random.uniform(97.2, 103.5, N_SAMPLES), 1),
    'spo2': np.random.randint(88, 100, N_SAMPLES),
    'pain_score_clinical': np.random.randint(0, 11, N_SAMPLES),
    
    # Labs
    'wbc': np.round(np.random.uniform(3.5, 18.0, N_SAMPLES), 1),
    'hemoglobin': np.round(np.random.uniform(8.0, 16.5, N_SAMPLES), 1),
    'platelet_count': np.random.randint(120000, 420000, N_SAMPLES),
    'sodium': np.round(np.random.uniform(130.0, 146.0, N_SAMPLES), 1),
    'potassium': np.round(np.random.uniform(3.2, 5.5, N_SAMPLES), 1),
    'creatinine': np.round(np.random.uniform(0.7, 3.5, N_SAMPLES), 2),
    'glucose': np.random.randint(75, 280, N_SAMPLES),
    'troponin': np.round(np.random.uniform(0.005, 1.2, N_SAMPLES), 3),
    'bnp': np.random.randint(20, 850, N_SAMPLES),
    'lactate': np.round(np.random.uniform(0.6, 4.2, N_SAMPLES), 1),
    'inr': np.round(np.random.uniform(0.9, 3.2, N_SAMPLES), 1),
    
    # Chronic conditions
    'diabetes_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.75, 0.25]),
    'hypertension_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.60, 0.40]),
    'cardiac_history_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.80, 0.20]),
    'copd_asthma_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.82, 0.18]),
    'ckd_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.88, 0.12]),
    'cancer_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.92, 0.08]),
    'immunocompromised_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.94, 0.06]),
    
    # Utilization & Demographics
    'chronic_condition_count': np.random.randint(0, 5, N_SAMPLES),
    'charlson_comorbidity_index': np.random.randint(0, 7, N_SAMPLES),
    'active_medication_count': np.random.randint(0, 12, N_SAMPLES),
    'on_anticoagulants_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.85, 0.15]),
    'on_insulin_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.88, 0.12]),
    'days_since_last_ed_visit': np.random.randint(5, 365, N_SAMPLES),
    'ed_visits_past_year': np.random.randint(0, 8, N_SAMPLES),
    'admissions_past_year': np.random.randint(0, 4, N_SAMPLES),
    'has_pcp_flag': np.random.choice([0, 1], N_SAMPLES, p=[0.20, 0.80]),
...
# 2. Critical vitals (hypotension, hypoxia < 92, high fever)
# 3. Elevated troponin (>= 0.04) or high lactate (>= 2.0)
# 4. Severe pain (>= 8) with cardiac/stroke signs
...
is_emergency = (
    ...
    (df['troponin'] >= 0.04) |
    (df['lactate'] >= 2.0) |
    (df['pain_level_self_reported'] >= 8) & (df['cardiac_history_flag'] == 1)
)
...
# Feature Engineering (matches ml_pipeline_avoidable_ed.py)
...
df['age_group'] = pd.cut(df['age'], bins=[0, 18, 40, 65, 120], labels=[0, 1, 2, 3]).astype(int)
...
print(f"\nModel Evaluation (scikit-learn {rf.__module__}):")
...
print("\nModel training and packaging completed successfully!")
